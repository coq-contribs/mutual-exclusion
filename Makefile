...
OCAMLLIBS?=
COQLIBS?= -R . MutualExclusion
COQDOCLIBS?=-R . MutualExclusion
...
VFILES:=binary/version1/Soundness.v\
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/MutualExclusion && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "MutualExclusion" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
