...
VFILES:=binary/version2/Correctness.v\
...
all: $(VOFILES) $(CMOFILES) $(if $(HASNATDYNLINK_OR_EMPTY),$(CMXSFILES)) ./binary/version1/mutex_prog\
...
./binary/version1/mutex_prog: ./binary/version1/Def.vo ./binary/version1/mutex_prog.ml
	$(CAMLOPTLINK) -ccopt -s -o $@ -I $$(ocamlfind query labltk) labltk.cmxa -I ./binary/version1/ ./mutex.mli ./mutex.ml ./binary/version1/mutex_prog.ml || echo 'Unsuccessful compilation of Ocaml extracted program'

./binary/version1/Mutex_prog: ./binary/version1/Def.vo ./binary/version1/Mutex_prog.hs
	which ghcxmake && ghcxmake $@ && strip $@ || echo 'Unsuccessful compilation of Haskell extracted program'
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/MutualExclusion/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/MutualExclusion/$$i; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/MutualExclusion/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/MutualExclusion/$$i;\
	done

uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/MutualExclusion && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "MutualExclusion" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/MutualExclusion \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find MutualExclusion/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
...
clean::
	...
	- rm -rf ./binary/version1/mutex_prog
	- rm -rf ./binary/version1/Mutex_prog
